...
def process_table_with_levels(df, table_num, api_name, level_cols, other_cols, conn, cur):
    ...
    for idx, row in df.iterrows():
        ...
        for lvl in sorted(current_levels):
            ...
            if full_path not in path_to_id:
                cols_sql = ['api_name', 'field_name', 'parent_field_id'] + [f'"{c}"' for c in other_cols]
                vals_sql = ["%s"] * (3 + len(other_cols))
                
                values = [api_name, name, parent_id] + [str(row[c]).strip() if c in row.index else "" for c in other_cols]
...
def process_table_without_levels(df, table_num, api_name, all_cols, conn, cur):
    ...
    for idx, row in df.iterrows():
        ...
        cols_sql = ['api_name', 'field_name', 'parent_field_id'] + [f'"{c}"' for c in all_cols]
        vals_sql = ["%s"] * (3 + len(all_cols))
        
        values = [api_name, field_name, None] + [str(row[c]).strip() if c in row.index else "" for c in all_cols]
...
def process_table(df, table_num, api_name, conn, cur):
    ...
    try:
        cur.execute('SELECT COUNT(*) FROM api_spec WHERE api_name = %s;', (api_name,))
        ...
        if existing_count > 0:
            ...
            if current_hash == existing_hash:
                ...
                cur.execute('DELETE FROM api_spec WHERE api_name = %s;', (api_name,))
                conn.commit()
    ...
    try:
        if level_cols:
            # Hierarchical table
            process_table_with_levels(df, table_num, api_name, level_cols, other_cols, conn, cur)
        else:
            # Flat table
            process_table_without_levels(df, table_num, api_name, all_cols, conn, cur)
...
def main():
    ...
    PDF_FILE = os.path.join(INPUT_FOLDER, files[0])
    BASE_API_NAME = os.path.splitext(files[0])[0]
    ...
    CREATE TABLE IF NOT EXISTS api_spec (
        ...
        api_name TEXT,
        field_name TEXT,
        parent_field_id INT REFERENCES api_spec(field_id)
    );
    """)
    conn.commit()
    print("✅ Table structure initialized")
    ...
    for table_num, table_df in enumerate(merged_tables, 1):
        ...
        try:
            process_table(table_df, table_num, api_name, conn, cur)
        except Exception as e:
